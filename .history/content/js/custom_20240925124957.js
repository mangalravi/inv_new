...
$(document).ready(function () {
  ...
  function syncPosition(el) {
    ...
    if (!thumbs.find(".owl-item.prev").length) {
  ...
  function removePrevClasses() {
    ...
    const prevCount = 3; // Number of prev classes to remove
  
    let removeCount = 0; // Track removed classes
  
    // Remove prev classes in reverse order
    for (let i = prevCount; i > 0; i--) {
      setTimeout(() => {
        items.eq(prevCount - i).removeClass("prev");
        removeCount++;
  
        // After removing all "prev" classes, check if any remain
        if (removeCount === prevCount && !thumbs.find(".owl-item.prev").length) {
          // Only then trigger the next slide
          bigimage.trigger('next.owl.carousel');
        }
      }, (prevCount - i + 1) * 400); // Timing for removal
    }
  }
