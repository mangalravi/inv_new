...
$(document).ready(function () {
  ...
  function removePrevClasses() {
    ...
    for (let i = prevCount; i > 0; i--) {
      setTimeout(() => {
        items.eq(prevCount - i).removeClass("prev"); // Remove the "prev" class
        removeCount++;
  
        // Check if all "prev" classes have been removed
        if (removeCount === prevCount) {
          // Reset the flags
          isRemovingPrevClasses = false;
          cycleComplete = false;
  
          // Trigger the next slide after all "prev" classes are removed
          bigimage.trigger('next.owl.carousel');
        }
      }, (prevCount - i + 1) * 400); // Adjust timing for removal
    }
  }
