...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let currentIndex = 0;
  let slideDuration = 3000; // Duration of each slide
  ...
  playButton.addEventListener("click", function () {
    ...
    }, slideDuration); // Resume autoplay with the original duration
    playButton.style.display = "none";
    pauseButton.style.display = "flex"; 
  });

  bigimage.on("changed.owl.carousel", function(event) {
    currentIndex = event.item.index; // Update current index on change
  });
  ...
  // Start autoplay interval when the page loads
  autoplayInterval = setInterval(() => {
    bigimage.trigger('next.owl.carousel');
  }, slideDuration);
