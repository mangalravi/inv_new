...
$(document).ready(function () {
  ...
  thumbs.owlCarousel({
    ...
    e.preventDefault();
    var number = $(this).index();
    bigimage.data("owl.carousel").to(number, 300, true); // Navigate to the selected slide
  ...
  var initialIndex = bigimage.find('.owl-item.active').index();
  thumbs.find(".owl-item").removeClass("current");
  thumbs.find(".owl-item").eq(initialIndex).addClass("current");
});
