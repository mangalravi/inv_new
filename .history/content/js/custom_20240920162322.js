...
$(document).ready(function() {
    ...
    pauseButton.addEventListener("click", function () {
        if (isPlaying) {
            // Stop the autoplay
            bigimage.trigger('stop.owl.autoplay'); 
            lastSlideTime = Date.now(); // Record the time of the last slide change
            pauseButton.style.display = "none";
            playButton.style.display = "flex";
            isPlaying = false;
        }
    ...
    playButton.addEventListener("click", function () {
        if (!isPlaying) {
            ...
            // Calculate the time elapsed since the last slide change
            var elapsedTime = Date.now() - lastSlideTime;
            var timeRemaining = autoplaySpeed - elapsedTime;

            // If the elapsed time exceeds the autoplay speed, play immediately
            if (timeRemaining <= 0) {
                timeRemaining = 0;
            }
