...
$(document).ready(function () {
  // Define an array of objects containing image sources and descriptions
  var slides = [
    { src: "content/images/banner1-thumbnail.jpg", alt: "Image 1", caption: "Next-Gen Utility for Smarter Cities!" },
    { src: "content/images/banner2-thumbnail.jpg", alt: "Image 2", caption: "Innovative Solutions for Modern Living!" },
    { src: "content/images/banner3-thumbnail.jpg", alt: "Image 3", caption: "Transforming Urban Landscapes!" },
    { src: "content/images/banner4-thumbnail.jpg", alt: "Image 4", caption: "Pioneering Sustainable Technology!" }
  ];

  // Append slides to the carousel container
  var $carouselContainer = $(".loop");
  slides.forEach(function(slide) {
    var $slide = $('<div class="item"></div>'); // Create a new slide
    $slide.append('<img src="' + slide.src + '" alt="' + slide.alt + '">'); // Add image
    $slide.append('<span>' + slide.caption + '</span>'); // Add caption
    $carouselContainer.append($slide); // Append to the carousel container
  });

  // Initialize Owl Carousel with custom dots
  var $owlCarousel = $carouselContainer.owlCarousel({
    center: true,
    items: 1,
    loop: true,
    margin: 20,
    dots: true,
    dotsData: true, // Use custom dot data
    autoplay: false,
    autoplayTimeout: 2500,
    autoplayHoverPause: true,
    smartSpeed: 800
  ...
  // Customize the dots after initialization
  $owlCarousel.on('initialized.owl.carousel', function(event) {
    var dots = event.page.pagerLinks;
    $(".owl-dots").empty(); // Clear default dots

    dots.forEach(function(dot, index) {
      var dotHtml = `
        <button role="button" class="owl-dot" data-index="${index}">
          <img src="${slides[index].src}" alt="Image ${index + 1}">
          <span>${slides[index].caption}</span>
        </button>
      `;
      $(".owl-dots").append(dotHtml); // Add custom dot HTML
    });

    // Set the initial active dot
    $(".owl-dots .owl-dot").eq(event.page.index).addClass('active');
  });
  ...
  // Update active class on dots when carousel changes
  $owlCarousel.on('changed.owl.carousel', function(event) {
    var currentIndex = event.page.index;
    $(".owl-dots .owl-dot").removeClass('active');
    $(".owl-dots .owl-dot").eq(currentIndex).addClass('active');
  });
});
