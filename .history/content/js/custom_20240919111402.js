...
$(document).ready(function () {
  ...
  var thumbs = $("#thumbs");
  var syncedSecondary = true;
  var isThumbnailClicked = false; // Track if a thumbnail has been clicked

  // Initialize bigimage carousel
  bigimage.owlCarousel({
    items: 1,
    nav: true,
    autoplay: true,
    autoplayTimeout: 5000, // Time before the next slide
    dots: false,
    loop: true,
    animateOut: "fadeOut",
    animateIn: "fadeIn",
    ...
    if (!isThumbnailClicked) {
      // Only update if a thumbnail hasn't been clicked
      thumbs.find(".owl-item").removeClass("current");
      var thumbIndex = currentIndex % thumbs.find(".owl-item").length;
      thumbs.find(".owl-item").eq(thumbIndex).addClass("current");
    }
  });
  ...
  // Synchronize secondary carousel with bigimage
  function syncPosition2(el) {
    if (syncedSecondary) {
      var number = el.item.index;
      bigimage.data("owl.carousel").to(number, 100, true);
      
      // Update current class on click
      thumbs.find(".owl-item").removeClass("current");
      thumbs.find(".owl-item").eq(number).addClass("current");
    }
  }

  // Click event on thumbs
  thumbs.on("click", ".owl-item", function (e) {
    e.preventDefault();
    var number = $(this).index();
    bigimage.data("owl.carousel").to(number, 300, true);
    
    // Update current class when thumbnail is clicked
    thumbs.find(".owl-item").removeClass("current");
    $(this).addClass("current"); // Add current class to the clicked thumbnail

    // Set flag indicating a thumbnail has been clicked
    isThumbnailClicked = true;
  });

  // Reset thumbnail click flag after a new slide is shown
  bigimage.on("changed.owl.carousel", function () {
    if (isThumbnailClicked) {
      // Optionally reset the flag after some time if desired
      setTimeout(() => {
        isThumbnailClicked = false;
      }, 5000); // Reset after the autoplay duration
    }
  });
});
