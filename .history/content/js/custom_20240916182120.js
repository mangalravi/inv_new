...
$(document).ready(function () {
      ...
      var $button = $(`
        <button role="button" class="owl-dot${index === 0 ? ' active' : ''}" data-index="${index}">
          <div class="dot-image-wrapper">
            ...
            <img src="${content.imgSrc}" alt="Image ${index + 1}"><img src="${content.imgSrc}" alt="Image ${index + 1}">
            </div>
          </div>
          <span>${content.text}</span>
        </button>
  ...
  function startProgress() {
    // Start interval to update progress circle
    window.progressInterval = setInterval(updateProgress, 2500); // Match with autoplayTimeout
  }
  ...
  function updateProgress(event) {
    ...
    $dots.each(function (index) {
      var $dot = $(this);
      var $circleProgress = $dot.find('.circle-progress');
      if (index === activeIndex) {
        $circleProgress.addClass('active');
      } else {
        $circleProgress.removeClass('active');
      }
    });
  }
});
