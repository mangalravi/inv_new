...
jQuery(function () {
  ...
  const $timelineSlider = jQuery(".timeline .slick-slider").slick({
      slidesToShow: 1,
      slidesToScroll: 1,
      arrows: false,
      dots: true,
  ...
  // Initialize the vertical year slider
  const $yearSlider = jQuery(".yeartimeslide").slick({
      vertical: true,
      infinite: false,
      centerMode: true,
      centerPadding: '20px',
  ...
  // Handle mouse wheel event for both sliders
  jQuery(window).on("wheel", function (e) {
      e.preventDefault();

      // Get current slide index for both sliders
      const currentTimelineSlide = $timelineSlider.slick('slickCurrentSlide');
      const totalTimelineSlides = $timelineSlider.slick('getSlick').slideCount;

      const currentYearSlide = $yearSlider.slick('slickCurrentSlide');
      const totalYearSlides = $yearSlider.slick('getSlick').slideCount;

      const delta = e.originalEvent.deltaY;

      // Determine which slider to scroll
      if (delta < 0) {
          // Scroll up
          if (currentYearSlide > 0) {
              $yearSlider.slick('slickPrev');
          } else if (currentTimelineSlide > 0) {
              $timelineSlider.slick('slickPrev');
          } else {
              // Allow page scroll if at the first slide of both
              window.scrollBy(0, -100); // Adjust scroll amount as needed
          }
      } else {
          // Scroll down
          if (currentYearSlide < totalYearSlides - 1) {
              $yearSlider.slick('slickNext');
          } else if (currentTimelineSlide < totalTimelineSlides - 1) {
              $timelineSlider.slick('slickNext');
          } else {
              // Allow page scroll if at the last slide of both
              window.scrollBy(0, 100); // Adjust scroll amount as needed
          }
      }
  });
