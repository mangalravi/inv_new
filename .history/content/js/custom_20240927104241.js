...
$(function() {
  let lastTransform = 'translate3d(0, 0, 0)'; // Initialize variable to hold the last transform value
  ...
  $("#gallery").on('mouseenter', function() {
    ...
    lastTransform = $(this).find('.slick-track').css('transform');
    $(this).find('.slick-track').css('transform', lastTransform); // Apply current transform
  }).on('mouseleave', function() {
    // Resume the slider
    $(this).slick('slickPlay');
    
    // Set the last transform value to continue from the last position
    $(this).find('.slick-track').css('transform', lastTransform); // Resume from last position
  });
});
...
$(function() {
  ...
  // Handle play/pause button click
  $('.play-pause').click(function() {
      $('.play-icon, .pause-icon').toggle(); // Toggle visibility of play/pause icons
  });
});
