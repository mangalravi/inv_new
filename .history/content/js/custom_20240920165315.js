...
$(document).ready(function() {
  ...
  var autoplayTime = 3000; // Duration for each slide
  var autoplayTimeout; // Store timeout ID
  var isPlaying = true; // To track play/pause state
  var startTime; // Timestamp when slide starts
  var remainingTime = autoplayTime; // Time remaining for the current slide

  function startAutoplay() {
    // If the slide is playing, use the remaining time
    autoplayTimeout = setTimeout(function() {
      bigimage.trigger('next.owl.carousel');
      remainingTime = autoplayTime; // Reset remaining time after moving to the next slide
      startAutoplay();
    }, remainingTime);
  }
  ...
  bigimage.owlCarousel({
    ...
    syncPosition(event);
    startTime = new Date().getTime(); // Reset start time when slide changes
    remainingTime = autoplayTime; // Reset remaining time on change
  });
  ...
  $('#pause-button').on('click', function() {
    ...
    remainingTime -= (new Date().getTime() - startTime) % autoplayTime;
  });

  // Initialize autoplay and start time
  startTime = new Date().getTime(); // Initialize the start time
