...
$(function() {
  ...
  function interval() {
      ...
      percentTime += 1 / (time * 10); // Increment the progress
      $('.slick-current .progress').css({
      ...
      if (percentTime >= 100) {
          $('.slider').slick('slickNext');
          percentTime = 0; // Reset percentTime for the next slide
          startProgressbar();
      }
  }
  ...
  $('.play-pause').click(function() {
      ...
      if (isPause) {
          ...
          // Calculate remaining time and adjust percentTime
          var remainingTime = (100 - percentTime) / 100 * time * 10; // Remaining progress in 10ms increments
          percentTime = 0; // Reset percentTime to start over

          startProgressbar(); // Restart progress bar with adjusted time
          $(".thumbnails .owl-item.active").removeClass("paused");
      }
  });
});
