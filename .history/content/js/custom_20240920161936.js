...
$(document).ready(function() {
  ...
  $(document).ready(function() {
    ...
    playButton.addEventListener("click", function () {
        if (!isPlaying) {
            // Get the current index using the Owl Carousel API
            var currentIndex = bigimage.find('.owl-item.active').index();
            var totalItems = bigimage.find('.owl-item').length;

            // Calculate elapsed time based on the current index
            var timeElapsed = currentIndex * autoplaySpeed;
            console.log("timeElapsed =>", timeElapsed);
            console.log("autoplaySpeed =>", autoplaySpeed);

            // Restart the autoplay from the remaining time
            bigimage.trigger('play.owl.autoplay', [autoplaySpeed - (timeElapsed % autoplaySpeed)]); 
            playButton.style.display = "none";
            pauseButton.style.display = "flex"; 
            isPlaying = true;
        }
