...
$('#gallery').on('init', function(event, slick, direction){
  const observer = new MutationObserver(function(mutationsList, observer){
    for(const mutation of mutationsList) {
      if (mutation.type === 'attributes' &&  mutation.attributeName == 'style' && freeze) {
      	$('#gallery .slick-track').css('transform', currentState);
      }
    }
  });
  observer.observe($('#slider .slick-track')[0], { attributes: true });
  $('#gallery .slick-track').mouseover(function() {
    styleBackup = $('#slider .slick-track').attr('style');
    currentState = getComputedStyle($('#slider .slick-track')[0]).transform;
    freeze = true;        
    ...
    if((m = regex.exec(styleBackup)) !== null) {
      ...
      let slideWidth = $('#slider .slick-slide').first().width();
  ...
  $('#gallery .slick-track').mouseout(function() {     
    freeze = false;
    $('#gallery .slick-track').attr('style', styleBackup.replace('5000ms', restOfTimeout+'ms'));
