...
$(document).ready(function () {
  ...
  var cycleComplete = false; // Track if a full cycle is complete
  ...
  function syncPosition(el) {
    ...
    // Check if cycle is complete and if we are not removing classes
    if (cycleComplete && current === 0 && !isRemovingPrevClasses) {
        removePrevClasses();
    } else if (!isRemovingPrevClasses) {
        // Only allow the next slide if we are not in the process of removing classes
        bigimage.trigger('next.owl.carousel');
    }

    cycleComplete = current === count;
}


  function addPrevClasses(current) {
    const items = thumbs.find(".owl-item");
    const prevCount = Math.min(3, current); // Limit to 3 items

    // Add prev class to the last 3 items before current
  ...
  function removePrevClasses() {
    const items = thumbs.find(".owl-item");
    const prevCount = 3; // Always remove 3 items if they exist
  
    isRemovingPrevClasses = true; // Set the flag to true
  
    let removeCount = 0; // To track the number of removed classes
  
    // Remove prev class in reverse order (3rd, 2nd, 1st)
    for (let i = prevCount; i > 0; i--) {
      setTimeout(() => {
        items.eq(prevCount - i).removeClass("prev"); 
        removeCount++;
  
        // After removing the last "prev" class
        if (removeCount === prevCount) {
          // Reset the flag and cycle state
          isRemovingPrevClasses = false; 
          cycleComplete = true; // Reset the cycle state for the next cycle

          // Allow the next slide to trigger only if all classes are removed
          bigimage.trigger('next.owl.carousel');
        }
      }, (prevCount - i) * 400); // Adjust timing to ensure sequence
    }
}
