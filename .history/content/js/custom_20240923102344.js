...
$(document).ready(function() {
  ...
  var lastChanged = 0; // To keep track of last change time

  // Initialize the bigimage carousel
  ...
  $(".owl-play").click(function() {
    var elapsedTime = Date.now() - lastChanged; // Calculate elapsed time since last slide change
    var remainingTime = autoplaySpeed - (elapsedTime % autoplaySpeed); // Calculate remaining time

    // Go to current slide immediately
    bigimage.trigger('to.owl.carousel', [bigimage.find('.owl-item.active').index(), 0, true]);

    // Start autoplay with the remaining time
    bigimage.trigger('play.owl.autoplay', [remainingTime]);
  });
