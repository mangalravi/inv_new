...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    // Calculate the total increments needed to reach either max or min size
    let totalIncrements = Math.ceil((maxSizeForElement - minSizeForElement) / Math.abs(change));
    
    // If the currentSize is closer to minSizeForElement, adjust increment size accordingly
    ...
    // Adjust the increment size for future clicks
    if (Math.abs(newSize - minSizeForElement) < (maxSizeForElement - minSizeForElement) / totalIncrements) {
      change = (maxSizeForElement - minSizeForElement) / totalIncrements;
    }
  });
}

function getIncrementSize(originalSize, currentSize, minSizeForElement, maxSizeForElement, steps = 2) {
  let totalSizeRange = maxSizeForElement - minSizeForElement;
  let currentProgress = currentSize - minSizeForElement;
  let remainingSteps = steps - Math.ceil(currentProgress / (totalSizeRange / steps));
  return totalSizeRange / remainingSteps;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleMenu(event) {
    ...
    const relatedUl = parentLi.querySelector("ul.selected");
