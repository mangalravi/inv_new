...
document.addEventListener("DOMContentLoaded", () => {
  ...
  checkCookieConsent();
  applyStoredTheme();
  handleResponsiveStyles(); // Ensure styles are applied based on the viewport size
  applyStoredFontSizes(); // Apply stored font sizes
...
function getResponsiveCSSFile() {
  ...
  if (viewportWidth < 768) return "content/css/Responsive-Mobile.css";
  if (viewportWidth >= 768 && viewportWidth <= 991) return "content/css/Responsive-Tablet.css";
