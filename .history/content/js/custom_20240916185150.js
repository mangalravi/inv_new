...
//     // Create rows and append columns with buttons
//     var $row;
//     dotContent.forEach(function(content, index) {
//       if (index % 4 === 0) {
//         // Create a new row every 4 columns
//         $row = $('<div class="row w-100"></div>');
//         $dotsContainer.append($row);
//       }

//       var $col = $('<div class="col-md-3"></div>'); // Create a column
//       var $button = $(`
//         <button role="button" class="owl-dot${index === 0 ? ' active' : ''}" data-index="${index}">
//           <img src="${content.imgSrc}" alt="Image ${index + 1}">
//           <span>${content.text}</span>
//         </button>
//       `);
//       $col.append($button); // Append button to column
//       $row.append($col); // Append column to row
//     });

//     // Ensure the first dot has the active class
//     $dotsContainer.find(".owl-dot:first").addClass("active");

//     // Attach click event to dots
//     $dotsContainer.find(".owl-dot").on('click', function() {
//       var index = $(this).data('index');
//       owl.trigger('to.owl.carousel', [index, 300]); // Move to the corresponding slide
//     });
//   }
// });
...
//     onInitialized: function (event) {
//       updateDots(event);
//       updateActiveDot(event); // Ensure the first dot is active on initialization
//     },
//     onTranslate: function (event) {
...
//     var $dotsContainer = $(event.target).find(".owl-dots");
//     var $dots = $dotsContainer.find(".owl-dot");
//     var activeIndex = $(event.target).find(".owl-item.active").data('owl-item') || 0;
...
//     var $dotsContainer = $(event.target).find(".owl-dots");
//     var $dots = $dotsContainer.find(".owl-dot");
//     var activeIndex = $(event.target).find(".owl-dot.active").data('index');
...
$(document).ready(function () {
  var owl = $(".loop").owlCarousel({
    ...
    onTranslate: function (event) {
      updateActiveDot(event); // Update active dot on translate
    },
    onTranslated: function (event) {
      // Optional: Can be used for additional features or animations
    },
  });
  ...
  // Ensure the first dot has the active class on page load
  $(".owl-dots .owl-dot").first().addClass('active');
});
