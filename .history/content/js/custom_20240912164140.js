...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    // Update font size using CSS classes
    updateFontSizeClass(element, newSize);
...
function updateFontSizeClass(element, size) {
  ...
  // Determine new size class based on the size
  if (size <= 14) {
...
function resetSize(className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    const originalSize = getOriginalSize(element);
    updateFontSizeClass(element, originalSize); // Reset font size using CSS classes
