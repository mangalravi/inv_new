...
function storeOriginalSize(element) {
  ...
  if (!localStorage.getItem(key)) {
    ...
    localStorage.setItem(key, computedSize / responsiveSize);
  }
}
...
function getOriginalSize(element) {
  ...
  const storedSize = localStorage.getItem(key);
  const responsiveSize = getResponsiveFontSize();
  return storedSize ? parseFloat(storedSize) * responsiveSize : getFontSize(element);
}
...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    // Save the new font size in localStorage
    const responsiveSize = getResponsiveFontSize();
    localStorage.setItem(getElementStorageKey(element), newSize / responsiveSize);
...
function applyStoredFontSizes() {
  document.querySelectorAll(".sec-head, .main-para, .liItem, .hyperelink, .cardttl, .cardpara").forEach((element) => {
    ...
    const responsiveSize = getResponsiveFontSize();
    if (storedSize) {
      element.style.fontSize = parseFloat(storedSize) * responsiveSize + "px";
    } else {
      const computedSize = getFontSize(element);
      localStorage.setItem(getElementStorageKey(element) + "-originalSize", computedSize / responsiveSize);
      element.style.fontSize = computedSize + "px";
    }
  });
}
