...
$(document).ready(function() {
    var bigimage = $("#big");
    var autoplaySpeed = 3000; // Set autoplay speed
    var isPlaying = true;

    // Initialize the Owl Carousel
    bigimage.owlCarousel({
      items: 1,
      slideSpeed: autoplaySpeed,
      nav: true,
      autoplay: true,
      dots: false,
      loop: true,
    ...
    pauseButton.style.display = "flex"; // Show pause button by default
    // Store the autoplay state
    pauseButton.addEventListener("click", function () {
      if (isPlaying) {
          // Stop the autoplay
          bigimage.trigger('stop.owl.autoplay'); 
          lastSlideTime = Date.now(); // Record the time when paused
          pauseButton.style.display = "none";
          playButton.style.display = "flex";
          isPlaying = false;
      }
  ...
  playButton.addEventListener("click", function () {
      if (!isPlaying) {
          ...
          var currentIndex = bigimage.find('.owl-item.active').index();
          var totalItems = bigimage.find('.owl-item').length;

          // Calculate elapsed time since the last slide change
          var elapsedTime = Date.now() - lastSlideTime;
          var timeRemaining = autoplaySpeed - elapsedTime;

          // If the elapsed time is greater than the autoplay speed, set it to 0
          if (timeRemaining <= 0) {
              timeRemaining = 0;
          }

          // Restart the autoplay from the remaining time
          bigimage.trigger('play.owl.autoplay', [timeRemaining]); 
          playButton.style.display = "none";
          pauseButton.style.display = "flex"; 
          isPlaying = true;

          // Update lastSlideTime to account for the new play action
          lastSlideTime = Date.now() + timeRemaining; // Adjust to new starting time
      }
  });

  // Update lastSlideTime on every slide change
  bigimage.on('changed.owl.carousel', function(event) {
      lastSlideTime = Date.now(); // Reset the time on every slide change
  });
});
