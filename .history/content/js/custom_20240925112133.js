...
$(document).ready(function () {
  ...
  function syncPosition(el) {
    ...
    // Update the cycleComplete state
  ...
  function removePrevClasses() {
    ...
    for (let i = prevCount; i > 0; i--) {
      setTimeout(() => {
        items.eq(prevCount - i).removeClass("prev"); // Remove in reverse order
        // After removing the last "prev" class, check for next cycle
        if (i === 1) {
          setTimeout(() => {
            bigimage.trigger("next.owl.carousel"); // Trigger the next slide after 1.2 seconds
            isRemovingPrevClasses = true; // Reset the flag
            cycleComplete = false; // Reset the cycle state for the next cycle
          }, 1200); // Delay for next slide
        }
      }, (prevCount + i) * 400); // 400ms delay for each item
    }
  }
