...
document.addEventListener("DOMContentLoaded", function () {
  const playButton = document.querySelector(".owl-play");
  const pauseBdocument.addEventListener("DOMContentLoaded", function () {
    const playButton = document.querySelector(".owl-play");
    const pauseButton = document.querySelector(".owl-pause");
    const bigimage = $("#big");
    ...
    pauseButton.addEventListener("click", function () {
        ...
        pauseButton.style.display = "none";
        playButton.style.display = "flex";
        clearInterval(progressInterval); // Stop the progress bars
    });
    ...
    playButton.addEventListener("click", function () {
        ...
        playButton.style.display = "none";
        pauseButton.style.display = "flex";
        startProgressBar(bigimage.trigger('current.owl.carousel').index); // Start for current slide
    });

    function startProgressBar(currentIndex) {
        ...
        // Duration of each slide in milliseconds (adjust as needed)
        const duration = 3000; // 3 seconds
        const currentProgress = progressBars[currentIndex];

        progressInterval = setInterval(() => {
            const currentWidth = parseFloat(currentProgress.style.width) || 0;
            const newWidth = currentWidth + (100 / (duration / 100)); // Increment the width
            
            if (newWidth >= 100) {
                currentProgress.style.width = '100%'; // Complete the progress bar
    ...
    $(document).ready(function () {
        ...
        bigimage.owlCarousel({
            ...
            startProgressBar(event.item.index); // Start progress bar for new slide
            syncPosition(event);
        });
...
utton = document.querySelector(".owl-pause");
  const bigimage = $("#big");
  ...
  $(document).ready(function () {
      ...
      bigimage.owlCarousel({
          ...
          startProgressBar(); // Restart progress bar on slide change
          syncPosition(event);
      });
