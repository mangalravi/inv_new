...
function initSmoothScrolling(container, animation) {
  ...
  // Calculate total width of the slides
  $('>div>div', container).each(function() {                
    animationWidth += $(this).outerWidth(false);        
  });

  // Detect number of visible slides
  var slidesVisible = Math.floor($(container).width() / $('>div>div:first-of-type', container).outerWidth(false));  
  slidesVisible = Math.max(slidesVisible, 1); // Ensure at least one slide is visible

  var slidesNumber = $('>div>div', container).length;
  var speed = slidesNumber * 2; // Determine animation speed based on the number of slides

  // Append cloned slides to create a seamless loop
  $('>div>div', container).slice(0, slidesVisible).clone().appendTo($('>div', container));  

  // Recalculate the total width of the slider
  $('>div>div', container).each(function() {
  ...
  // Insert styles for the animation
  $("<style type='text/css'>@keyframes " + animation + " { 0% { margin-left: 0px; } 100% { margin-left: -" + animationWidth + "px; } } " +
    ...
    "animation: " + animation + " " + speed + "s linear infinite; }</style>").appendTo("head");  

  // Restart the animation to ensure it works correctly across all browsers
  var cl = $(container).attr("class");
  $(container).removeClass(cl).animate({'nothing': null}, 1, function () {
    $(this).addClass(cl);
  });

  console.log("Animation initialized with speed: " + speed + "s and total width: " + animationWidth);
}
