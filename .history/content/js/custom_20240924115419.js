...
$(document).ready(function () {
  ...
  var autoplayDuration = 5000; // Duration of each slide in milliseconds (5 seconds)
  var autoplayTimeout; // Variable to manage autoplay timing
  var isPlaying = true; // Track autoplay state
  var currentIndex = 0; // Track current index
  ...
  bigimage
    ...
    .on("changed.owl.carousel", function (event) {
      currentIndex = event.item.index; // Update the current index
      syncPosition(event);
    });
  ...
  $(".play").on("click", function () {
    if (!isPlaying) {
      ...
      bigimage.trigger("play.owl.autoplay", [autoplayDuration]); // Start autoplay

      // Calculate the remaining time on the current slide
      const remainingTime = autoplayDuration - (Date.now() % autoplayDuration);

      // Set timeout to change to the next slide after the remaining time
      clearTimeout(autoplayTimeout);
      autoplayTimeout = setTimeout(function () {
        bigimage.trigger("next.owl.carousel"); // Move to the next slide
        bigimage.trigger("play.owl.autoplay", [autoplayDuration]); // Restart autoplay
      }, remainingTime);
    }
  });
  ...
  // Clear timeout when stopping autoplay
  bigimage.on("stop.owl.autoplay", function () {
    clearTimeout(autoplayTimeout);
  });

  // Handle the autoplay timing when clicking play
  bigimage.on("play.owl.autoplay", function () {
    if (autoplayTimeout) clearTimeout(autoplayTimeout); // Clear any existing timeout
    autoplayTimeout = setTimeout(function () {
      bigimage.trigger("next.owl.carousel"); // Move to the next slide
      bigimage.trigger("play.owl.autoplay", [autoplayDuration]); // Restart autoplay
    }, autoplayDuration); // Set the timeout to the slide duration
  });
  ...
  bigimage.on("stop.owl.autoplay", function () {
    if (autoplayTimeout) {
      clearTimeout(autoplayTimeout);
      autoplayTimeout = null; // Reset timeout
    }
  });
