...
function storeOriginalSize(element) {
  const key = getElementStorageKey(element) + `-${getResponsiveCSSFile()}`;
...
function getResponsiveCSSFile() {
  // Determine which responsive CSS file is applied based on the viewport width
  const viewportWidth = window.innerWidth;
  if (viewportWidth < 768) {
      return 'Responsive-Mobile.css';
  } else if (viewportWidth >= 768 && viewportWidth <= 991) {
      return 'Responsive-Desktop.css';
  }
  return ''; // Default or larger screen size
}
...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
      ...
      localStorage.setItem(getElementStorageKey(element) + `-${getResponsiveCSSFile()}`, newSize);
...
function resetSize(className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
      ...
      localStorage.removeItem(getElementStorageKey(element) + `-${getResponsiveCSSFile()}`);
  });
}
...
function applyStoredFontSizes() {
  document.querySelectorAll(".sec-head, .main-para, .liItem, .hyperelink, .cardttl, .cardpara").forEach((element) => {
      const storedSize = localStorage.getItem(getElementStorageKey(element) + `-${getResponsiveCSSFile()}`);
      if (storedSize) {
          element.style.fontSize = storedSize + "px";
      } else {
          const computedSize = getFontSize(element);
          localStorage.setItem(getElementStorageKey(element) + `-${getResponsiveCSSFile()}`, computedSize);
