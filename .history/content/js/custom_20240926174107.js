...
// $(function() {
//   var totalTime = 2000; // Total time for the slide in milliseconds (2 seconds)
//   var isPause = false;
//   var tick;
//   var elapsedTime = 0; // Track the elapsed time
//   var percentTime = 0; // Progress percentage
...
//   // Start progress bar for the current slide
//   function startProgressbar() {
//       tick = setInterval(interval, 45); // Update every 10ms
//   }

//   function interval() {
//       if (isPause) return;

//       elapsedTime += 10; // Increment elapsed time
//       percentTime = (elapsedTime / totalTime) * 100; // Calculate percentage

//       $('.slick-current .progress').css({
//           width: Math.min(percentTime, 100) + "%" // Ensure it does not exceed 100%
//       });

//       if (elapsedTime >= totalTime) {
//           $('.slider').slick('slickNext');
//           resetProgressbar(); // Reset progress for the next slide
//       }
//   }
...
//       $('.progress').css({ width: 0 + '%' });
//       clearTimeout(tick);
//       startProgressbar(); // Restart progress for the new slide
//   }
...
//           $('.play-icon').show();
//           $('.pause-icon').hide();
//           clearInterval(tick); // Stop the interval
//           $(".thumbnails .owl-item.active").addClass("paused");
//       } else {
//           $('.play-icon').hide(); 
//           $('.pause-icon').show(); 
...
$(function() {
  ...
  // Start progress bar for the current slide
  function startProgressbar() {
      tick = setInterval(interval, 10); // Update every 10ms
  }
  ...
  function interval() {
      ...
      $('.slider .slick-current .progress').css({
          width: Math.min(percentTime, 100) + "%" // Ensure it does not exceed 100%
      });
  ...
  function resetProgressbar() {
      elapsedTime = 0; // Reset elapsed time
      percentTime = 0; // Reset percent time
      $('.slider .slick-current .progress').css({ width: 0 + '%' });
      clearTimeout(tick);
      startProgressbar(); // Restart progress for the new slide
  }
