...
function addPrevClasses(current) {
  ...
  // Add "prev" class to the current item and the last 3 items before it
  items.removeClass("prev"); // Clear previous classes first
...
function removePrevClasses() {
  const items = thumbs.find(".owl-item");
  const prevCount = 4; // Remove the last 4 items if they exist

  isRemovingPrevClasses = true; // Set the flag to true

  // Remove prev class in reverse order (4th, 3rd, 2nd, 1st)
  let removeCount = 0; // To track the number of removed classes

  for (let i = 0; i < prevCount; i++) {
    setTimeout(() => {
      if (items.eq(i).hasClass("prev")) {
        items.eq(i).removeClass("prev");
        removeCount++;

        // After removing the last "prev" class, reset the flags and allow the next slide
    ...
    }, (i + 1) * 400); // Adjust timing for sequential removal
  }
}
