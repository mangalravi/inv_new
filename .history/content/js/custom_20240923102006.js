...
$(document).ready(function() {
  ...
  var autoplayInterval; // Variable to store autoplay interval
  var autoplaySpeed = 3000; // Set your desired speed here
  ...
  bigimage.owlCarousel({
    ...
    dots: false,
    loop: true,
    autoplayTimeout: autoplaySpeed, // Use this to set the autoplay timeout
  ...
  $(".owl-play").click(function() {
    ...
    var remainingTime = autoplaySpeed - ((Date.now() - bigimage.data('lastChanged')) % autoplaySpeed);

    // Restart the carousel with the remaining time
    bigimage.trigger('to.owl.carousel', [currentIndex, remainingTime, true]);
    bigimage.trigger('play.owl.autoplay', [autoplaySpeed]); // Start autoplay with defined speed
  });

  // Pause button click event
  $(".owl-pause").click(function() {
    bigimage.trigger('stop.owl.autoplay'); // Stop the autoplay
  });

  // Store the last change time
  bigimage.on("changed.owl.carousel", function(event) {
    bigimage.data('lastChanged', Date.now());
  });
});
