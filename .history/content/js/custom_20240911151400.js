...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleMenu(event) {
    ...
    // Find the related <ul> within the parent <div>
    const relatedUl = document.querySelector(".selected");
  ...
  // Add event listeners for both click and hover events on toggle images
  toggleImages.forEach((image) => {
    image.addEventListener("click", toggleMenu);
    image.addEventListener("mouseover", toggleMenu); // Optional: toggle on hover
    image.addEventListener("mouseout", () => {
      // Hide the menu when mouse leaves the .tglimg area
      const relatedUl = document.querySelector(".selected");
      if (relatedUl) {
        relatedUl.classList.remove("active");
      }
    });
  });
  ...
  document.addEventListener("click", (event) => {
    if (!event.target.closest(".d-flex")) {
      document.querySelectorAll(".d-flex .selected.active").forEach((ul) => {
