...
const lineHeightMaxSize = "3rem"; // Define the line height for maximum size
...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    // Check if the element is .main-para and if it has reached max size
    if (element.classList.contains("main-para")) {
      if (newSize >= maxSizeForElement) {
        element.style.lineHeight = lineHeightMaxSize;
      } else {
        // Optionally reset line height if below max size
        element.style.lineHeight = ""; // You can set this to some other value if needed
      }
    }
  });
}
...
function resetSize(className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    // Reset the line height for .main-para
    if (element.classList.contains("main-para")) {
      element.style.lineHeight = ""; // Reset to default or specify a default value
    }
  ...
  document
    ...
    .forEach((element) => {
      ...
      if (storedSize) {
        ...
        // Check if the element is .main-para and apply line height if needed
        if (element.classList.contains("main-para") && parseFloat(storedSize) >= getOriginalSize(element) * maxSizeMultiplier) {
          element.style.lineHeight = lineHeightMaxSize;
        }
      } else {
