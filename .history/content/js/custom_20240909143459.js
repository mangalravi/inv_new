const initialFontSize = 16; // This is just a base value; each element will use its own initial size.
...
function changeSizeByBtn(change, className) {
  ...
  document.querySelectorAll(className).forEach((element) => {
    ...
    let storedSize = localStorage.getItem(
      getElementStorageKey(element) + "-originalSize"
    );
    let originalSize = storedSize
...
function applyStoredFontSizes() {
  ...
  document
    ...
    .forEach((element) => {
      ...
      if (
        !localStorage.getItem(getElementStorageKey(element) + "-originalSize")
      ) {
