...
$(document).ready(function() {
  ...
  .on("changed.owl.carousel", function(event) {
    var items = $(".owl-item", bigimage);
    items.removeClass("active").css("opacity", 0); // Reset opacity
    items.eq(event.item.index).addClass("active").css("opacity", 1); // Fade in active item
  });
  ...
  $(".owl-item", bigimage).eq(0).addClass("active").css("opacity", 1); // Ensure the first item is visible
});
