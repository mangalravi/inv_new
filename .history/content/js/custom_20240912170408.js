...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("incbtn")
    .addEventListener("click", () => changeSizeByBtn(1));
  document
    .getElementById("decbtn")
    .addEventListener("click", () => changeSizeByBtn(-1));
  document
...
function updateCSSVariables(sizeChange) {
  ...
  Object.keys(variables).forEach((variable) => {
    const baseSize = variables[variable];
...
function changeSizeByBtn(change) {
  ...
  const currentSizeChange = getCurrentSizeChange();
  const newSizeChange = currentSizeChange + change * sizeChangePercentage;
  setSizeChange(newSizeChange);

  updateCSSVariables(newSizeChange);
}

function resetSize() {
  if (isOverlayVisible()) return;

  setSizeChange(0); // Reset to base size
  updateCSSVariables(0);
}

function getCurrentSizeChange() {
  // Retrieve the current size change from local storage or use 0 as default
  return parseFloat(localStorage.getItem("sizeChange")) || 0;
}

function setSizeChange(sizeChange) {
...
function applyStoredFontSizes() {
  // This function is updated to use CSS variables directly
  // Ensure your CSS is using these variables
}
